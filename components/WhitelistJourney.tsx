...
export function WhitelistJourney() {
  ...
  return <main className="tile-site">
    <nav className="nav">
      <button className="brand" onClick={() => setStage("home")}><span className="brand-mark">✿</span><span>TILE<small>타일 · 1111</small></span></button>
    ...
    {stage === "home" ? <>
      <section id="home" className="hero">
        <div className="hero-sun"/><div className="hero-pine hero-pine-a"/><div className="hero-pine hero-pine-b"/><div className="hero-film"/>
        <div className="hero-copy">
          <div className="hero-emblem">✿</div><p className="hero-ko-label">기와에서 시작되는 1,111개의 이야기</p><h1>TILE</h1>
      ...
      <section className="why1111">
        <div className="big-number">1111</div><div><p className="eyebrow">02 · WHY 1111? · 왜 1111?</p><h2>NOT A CROWD.<br/>A ROOF.</h2><p>1,111 is the shape of the first TILE collection: limited enough for every piece to carry identity, large enough to form a real community. Like a hanok roof, the meaning comes from individual pieces becoming one structure.</p><span className="korean-note">서로 다른 기와, 하나의 지붕.</span></div>
      </section>

      <section className="status-section">
        <p className="eyebrow">03 · COLLECTION STATUS · 현재 상태</p><div className="status-grid"><div><small>SUPPLY</small><strong>1,111</strong><span>UNIQUE NFTs</span></div><div><small>NETWORK</small><strong>ROBINHOOD</strong><span>CHAIN MAINNET</span></div><div><small>MINT DATE</small><strong>TBA</strong><span>TO BE ANNOUNCED</span></div><div><small>DESTINATION</small><strong>GIWA</strong><span>WHEN MAINNET IS LIVE</span></div></div>
      </section>

      <section className="wl-story">
        <div className="section-intro"><p className="eyebrow">04 · THE WHITELIST JOURNEY · 화이트리스트</p><h2>FIVE MOVES.<br/>ONE APPLICATION.</h2><p>The whitelist is a short participation flow, not a puzzle. Complete the signal, leave your wallet, share TILE, then verify your post.</p></div>
        <div className="wl-story-track">{[["01","FOLLOW","@TileOnGIWA",Users],["02","ENGAGE","Like · Repost · Tag 2",Sparkles],["03","APPLY","X + EVM Wallet",WalletCards],["04","SHARE","Prepared X Post",ArrowUpRight],["05","VERIFY","Paste Post URL",ShieldCheck]].map(([n,t,d,Icon]:any)=><div className="wl-story-step" key={String(n)}><span>{n}</span><Icon size={22}/><b>{t}</b><small>{d}</small></div>)}</div>
        <button className="wide-wl-cta" onClick={() => setStage("tasks")}>START WHITELIST APPLICATION <ArrowRight/></button>
      </section>

      <section id="journey" className="migration-section">
        <div className="migration-copy"><p className="eyebrow">05 · THE NETWORK JOURNEY · 네트워크 여정</p><h2>ROBINHOOD<br/>→ GIWA</h2><p>TILE begins on Robinhood Chain mainnet. When GIWA mainnet is live, the collection is planned to move to GIWA. That is where the next chapter begins — and where TILE holders will be rewarded.</p></div>
        <div className="migration-flow"><div className="network-node"><small>01 · MINT</small><strong>ROBINHOOD<br/>CHAIN</strong><span>MAINNET · DATE TBA</span></div><div className="transfer-line"><i/><ArrowRight/><i/></div><div className="network-node active"><small>02 · MIGRATION</small><strong>GIWA</strong><span>WHEN MAINNET IS LIVE</span></div><div className="transfer-line"><i/><ArrowRight/><i/></div><div className="network-node reward"><small>03 · HOLD</small><strong>REWARDS</strong><span>FOR TILE HOLDERS ON GIWA</span></div></div>
      </section>

      <section id="faq" className="faq-section"><div className="faq-title"><p className="eyebrow">06 · FAQ · 자주 묻는 질문</p><h2>THE IMPORTANT<br/>PARTS.</h2><p>Everything that should be clear before you apply.</p></div><div className="faq-list">{faqs.map(([q,a],i)=><article className={openFaq===i?"open":""} key={q}><button onClick={()=>setOpenFaq(openFaq===i?null:i)}><span>0{i+1}</span><b>{q}</b><ChevronDown/></button>{openFaq===i&&<p>{a}</p>}</article>)}</div></section>

      <section className="final-cta"><div className="foundation-mark">✿</div><div><p className="eyebrow">BE PART OF THE FOUNDATION · 함께 지붕을 만들다</p><h2>ONE TILE AT A TIME.</h2><p>1,111 pieces. Robinhood Chain first. GIWA next.</p></div><button onClick={() => setStage("tasks")}>JOIN WHITELIST <ArrowUpRight/></button></section>
